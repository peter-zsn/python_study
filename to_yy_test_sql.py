...
@file: tosql.py
@time: 2017/6/27 9:32
112中的u_yy_test中的小学英语作业， 转移到130中yy_test1中，数据量317486
"""
import MySQLdb
import time
import json
...
task_nums_sql = """
SELECT count(id) FROM u_yy_test;
                """
...
for i in range(page):
    ...
    SELECT id, user_id, catalog_id, nquestion, score, `status`, add_time, test_time, object_id
     FROM u_yy_test limit %s, 500;
        """ % start
    print sql
    big_cur.execute(sql)
    big_recode = big_cur.fetchall()
    my_detail = []
    for (id, user_id, catalog_id, nquestion, score, status, add_time, test_time, object_id) in big_recode:
        d = dict(
            id=id,
            user_id=user_id,
    ...
    insert_sql = "insert into yy_test1 (%s) VALUES (%s)" % (fields, tokens)
